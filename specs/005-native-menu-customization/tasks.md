...
**Prerequisites**: plan.md checked | spec.md checked | research.md checked | data-model.md checked | contracts/ checked | quickstart.md checked
...
- [x] T001 Add `MENU` channel group to `src/electron/ipc/channels.ts` with constants `TOGGLE_BOTTOM_PANEL`, `TOGGLE_SECONDARY_PANEL`, `THEME_CHANGED`, `UPDATE_PANEL_STATE`
...
## Phase 3: User Story 1 - Menu simple y utilizable en espanol (Priority: P1) MVP
...
- [x] T009 [US1] Wire `app.quit()` as the `click` handler for slot `file.exit` in `menu.defaults.ts`
- [x] T010 [US1] Wire panel toggle callbacks in the native menu for `view.bottomPanel` and `view.secondaryPanel`: main sends typed IPC events, and the renderer bridge executes the central shell command registry commands (e.g., `shell.panel.toggleBottom`) rather than mutating NgRx directly in the main process
...
- [x] T014 [US2] Add theme `click` handlers inside `MenuBuilder.build()` for `themes.dark` (and stub for `themes.light`): on click, set `nativeTheme.themeSource`, write `shell.theme` preference to disk via the existing `writeEnvelope` helper, rebuild and re-apply the menu via `Menu.setApplicationMenu()`, call `mainWindow.webContents.send(MENU.THEME_CHANGED, { theme })`
...
- [x] T026 [US1] Expose `MENU.UPDATE_PANEL_STATE` so the renderer can keep native menu checkbox state synchronized with NgRx layout visibility state
...
## Phase 5: User Story 3 - Personalizar menu sin tocar el nucleo (Priority: P3)
...
- [x] T018 [US3] Validate `MenuBuilder` enforces that `file.exit` cannot be hidden (silently ignores `visible: false` override for that slot); add guard in `menu.builder.ts`
...
- [x] T020 [US3] Write `specs/005-native-menu-customization/quickstart.md` validation: create `src/electron/menu/menu.builder.spec.ts` (Jasmine) covering: default entries present, override label applied, `file.exit` hidden-override ignored, `extraEntries` appended, `themes.light` always disabled
...
- [x] T022 [P] Add inline `// Future: enable when light theme spec ships` comment on the `themes.light` disabled guard in `menu.builder.ts`
...
| 4a Panel state sync | T026 | US1 | 0 |
| 4b US2 Perf (SC-002) | T027 | US2 | 0 |
